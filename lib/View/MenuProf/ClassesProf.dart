...
class classprofState extends StatefulWidget {
  final String valeur;
  classprofState(this.valeur);
  @override
  State<StatefulWidget> createState() {return classprof(this.valeur);}
}
class classprof extends State<classprofState> {
  final String valeur ;
  classprof(this.valeur);
              ...
              builder: (context, snapshot) {
                ...
                return ListView(
                  children: snapshot.data.map((data) => Column(children: <Widget>[
                          FadeAnimation(1.0,InfoCard(text:"Groupe: "+ data.nomGroupe+"    Matière: "+data.nom, icon: Icons.account_circle, onPressed: () async {Navigator.of(context).push(

                  MaterialPageRoute(
                      builder: (context) => DashboardClasses(data.nomGroupe,data.idMatiere.toString(),data.idGroupe.toString())),
                );})),
